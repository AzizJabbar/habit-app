...
class HabitRepository(private val habitDao: HabitDao, private val executor: ExecutorService) {
    ...
    // 4 : Use SortUtils.getSortedQuery to create sortable query and build paged list
    fun getHabits(filter: HabitSortType): LiveData<PagedList<Habit>> {
        val query = SortUtils.getSorteredQuery(filter)
        val pagedListConfig = PagedList.Config.Builder().setPageSize(30).build()
        return LivePagedListBuilder(
            habitDao.getHabits(query), pagedListConfig
        ).build()
    }

    // 5 : Complete other function inside repository
    fun getHabitById(habitId: Int): LiveData<Habit> {
        return habitDao.getHabitById(habitId)
    }

    fun insertHabit(newHabit: Habit){
        executor.execute{habitDao.insertHabit(newHabit)}
    }
